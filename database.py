...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS indicators (
        ...
        proposal_id INTEGER,
        indicator_text TEXT,
        indicator_text_embedding TEXT,
        objective_link TEXT,
        status TEXT DEFAULT 'draft',   -- draft | approved | rejected | edited
        created_at TEXT,
        FOREIGN KEY (proposal_id) REFERENCES proposals(id)
    )""")
    # Safe migration for a DB created before this column existed.
    try:
        cur.execute("ALTER TABLE indicators ADD COLUMN indicator_text_embedding TEXT")
    ...
    # Beneficiary-voice-vs-indicator divergence review. One row per
    # (indicator, feedback) pair a human has actually acted on — nothing
    # here is written until a human clicks a verification button.
    cur.execute("""
    CREATE TABLE IF NOT EXISTS dissonance_reviews (
        ...
        semantic_score REAL,
        lexical_score REAL,
        entailment_verdict TEXT,
        entailment_reasoning TEXT,
...
def reset_db():
    """Wipes all rows from every table for a clean demo restart, without
...
def set_indicator_embedding(indicator_id, embedding):
    conn = get_connection()
    conn.execute(
        "UPDATE indicators SET indicator_text_embedding = ? WHERE id = ?",
        (json.dumps(embedding), indicator_id),
    )
    conn.commit()
    conn.close()
...
# ---------- dissonance reviews (beneficiary voice vs indicator, human verification) ----------

def upsert_dissonance_review(indicator_id, feedback_id, semantic_score, lexical_score,
                              entailment_verdict, entailment_reasoning, status, human_note):
    """One review row per (indicator, feedback) pair — overwrites any prior
    review of the same pair rather than duplicating."""
    conn = get_connection()
    cur = conn.cursor()
    existing = cur.execute(
        "SELECT id FROM dissonance_reviews WHERE indicator_id = ? AND feedback_id = ?",
        (indicator_id, feedback_id),
    ).fetchone()
    if existing:
        cur.execute(
            "UPDATE dissonance_reviews SET semantic_score = ?, lexical_score = ?, "
            "entailment_verdict = ?, entailment_reasoning = ?, status = ?, human_note = ?, "
            "reviewed_at = ? WHERE id = ?",
            (semantic_score, lexical_score, entailment_verdict, entailment_reasoning,
             status, human_note, _now(), existing["id"]),
        )
    else:
        cur.execute(
            "INSERT INTO dissonance_reviews (indicator_id, feedback_id, semantic_score, "
            "lexical_score, entailment_verdict, entailment_reasoning, status, human_note, "
            "reviewed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (indicator_id, feedback_id, semantic_score, lexical_score, entailment_verdict,
             entailment_reasoning, status, human_note, _now()),
        )
    conn.commit()
    conn.close()


def get_dissonance_review(indicator_id, feedback_id):
    conn = get_connection()
    row = conn.execute(
        "SELECT * FROM dissonance_reviews WHERE indicator_id = ? AND feedback_id = ?",
        (indicator_id, feedback_id),
    ).fetchone()
    conn.close()
    return dict(row) if row else None
...
def get_dissonance_reviews_for_indicator(indicator_id):
    conn = get_connection()
    rows = conn.execute(
